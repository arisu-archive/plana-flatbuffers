...
class ClearDeckRuleExcelT(object):
    ...
    def __init__(
        ...
        self.sizeLimit = sizeLimit  # type: int

    @classmethod
    def InitFromBuf(cls, buf, pos):
        clearDeckRuleExcel = ClearDeckRuleExcel()
        clearDeckRuleExcel.Init(buf, pos)
        return cls.InitFromObj(clearDeckRuleExcel)
    ...
    @classmethod
    def InitFromObj(cls, clearDeckRuleExcel):
        x = ClearDeckRuleExcelT()
        x._UnPack(clearDeckRuleExcel)
        return x

    # ClearDeckRuleExcelT
    def _UnPack(self, clearDeckRuleExcel):
        if clearDeckRuleExcel is None:
            return
        self.contentType = clearDeckRuleExcel.ContentType()
        self.sizeLimit = clearDeckRuleExcel.SizeLimit()

    # ClearDeckRuleExcelT
    def Pack(self, builder):
        ClearDeckRuleExcelStart(builder)
        ClearDeckRuleExcelAddContentType(builder, self.contentType)
        ClearDeckRuleExcelAddSizeLimit(builder, self.sizeLimit)
        clearDeckRuleExcel = ClearDeckRuleExcelEnd(builder)
        return clearDeckRuleExcel

# plana-flatbuffer: object-api conversion
from FlatData._conversion import install_object_api as _install_object_api
_install_object_api(ClearDeckRuleExcelT, 'ClearDeckRuleExcel', (('contentType', 'int32', False), ('sizeLimit', 'int64', False)))
