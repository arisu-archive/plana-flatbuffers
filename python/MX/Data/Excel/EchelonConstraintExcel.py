...
class EchelonConstraintExcelT(object):
    ...
    def __init__(
        ...
        self.isWhiteList = isWhiteList  # type: bool
        self.characterId = characterId  # type: Optional[List[int]]
        self.personalityId = personalityId  # type: Optional[List[int]]
    ...
    def InitFromBuf(cls, buf, pos):
        echelonConstraintExcel = EchelonConstraintExcel()
        echelonConstraintExcel.Init(buf, pos)
        return cls.InitFromObj(echelonConstraintExcel)
    ...
    @classmethod
    def InitFromObj(cls, echelonConstraintExcel):
        x = EchelonConstraintExcelT()
        x._UnPack(echelonConstraintExcel)
        return x

    # EchelonConstraintExcelT
    def _UnPack(self, echelonConstraintExcel):
        if echelonConstraintExcel is None:
            return
        self.groupId = echelonConstraintExcel.GroupId()
        self.isWhiteList = echelonConstraintExcel.IsWhiteList()
        if not echelonConstraintExcel.CharacterIdIsNone():
            if np is None:
                self.characterId = []
                for i in range(echelonConstraintExcel.CharacterIdLength()):
                    self.characterId.append(echelonConstraintExcel.CharacterId(i))
            else:
                self.characterId = echelonConstraintExcel.CharacterIdAsNumpy()
        if not echelonConstraintExcel.PersonalityIdIsNone():
            if np is None:
                self.personalityId = []
                for i in range(echelonConstraintExcel.PersonalityIdLength()):
                    self.personalityId.append(echelonConstraintExcel.PersonalityId(i))
            else:
                self.personalityId = echelonConstraintExcel.PersonalityIdAsNumpy()
        self.weaponType = echelonConstraintExcel.WeaponType()
        self.school = echelonConstraintExcel.School()
        self.club = echelonConstraintExcel.Club()
        self.role = echelonConstraintExcel.Role()
    ...
    def Pack(self, builder):
        if self.characterId is not None:
            if np is not None and type(self.characterId) is np.ndarray:
                ...
                EchelonConstraintExcelStartCharacterIdVector(builder, len(self.characterId))
                for i in reversed(range(len(self.characterId))):
                    builder.PrependInt64(self.characterId[i])
                characterId = builder.EndVector()
        if self.personalityId is not None:
            if np is not None and type(self.personalityId) is np.ndarray:
                personalityId = builder.CreateNumpyVector(self.personalityId)
            else:
                EchelonConstraintExcelStartPersonalityIdVector(builder, len(self.personalityId))
                for i in reversed(range(len(self.personalityId))):
                    builder.PrependInt64(self.personalityId[i])
                personalityId = builder.EndVector()
        EchelonConstraintExcelStart(builder)
        EchelonConstraintExcelAddGroupId(builder, self.groupId)
        EchelonConstraintExcelAddIsWhiteList(builder, self.isWhiteList)
        if self.characterId is not None:
            EchelonConstraintExcelAddCharacterId(builder, characterId)
        if self.personalityId is not None:
            EchelonConstraintExcelAddPersonalityId(builder, personalityId)
        EchelonConstraintExcelAddWeaponType(builder, self.weaponType)
        EchelonConstraintExcelAddSchool(builder, self.school)
        EchelonConstraintExcelAddClub(builder, self.club)
        EchelonConstraintExcelAddRole(builder, self.role)
        echelonConstraintExcel = EchelonConstraintExcelEnd(builder)
