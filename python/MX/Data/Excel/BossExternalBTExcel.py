...
class BossExternalBTExcelT(object):
    ...
    def __init__(
        ...
        self.externalBtNodeType = externalBtNodeType  # type: int
        self.externalBtTrigger = externalBtTrigger  # type: int
        self.triggerArgument = triggerArgument  # type: Optional[str]
        self.behaviorRate = behaviorRate  # type: int
        self.externalBehavior = externalBehavior  # type: int
        self.behaviorArgument = behaviorArgument  # type: Optional[str]

    @classmethod
    def InitFromBuf(cls, buf, pos):
        bossExternalBtexcel = BossExternalBTExcel()
        bossExternalBtexcel.Init(buf, pos)
        return cls.InitFromObj(bossExternalBtexcel)
    ...
    @classmethod
    def InitFromObj(cls, bossExternalBtexcel):
        x = BossExternalBTExcelT()
        x._UnPack(bossExternalBtexcel)
        return x

    # BossExternalBTExcelT
    def _UnPack(self, bossExternalBtexcel):
        if bossExternalBtexcel is None:
            return
        self.externalBtId = bossExternalBtexcel.ExternalBtId()
        self.aiPhase = bossExternalBtexcel.AiPhase()
        self.externalBtNodeType = bossExternalBtexcel.ExternalBtNodeType()
        self.externalBtTrigger = bossExternalBtexcel.ExternalBtTrigger()
        self.triggerArgument = bossExternalBtexcel.TriggerArgument()
        self.behaviorRate = bossExternalBtexcel.BehaviorRate()
        self.externalBehavior = bossExternalBtexcel.ExternalBehavior()
        self.behaviorArgument = bossExternalBtexcel.BehaviorArgument()

    # BossExternalBTExcelT
    def Pack(self, builder):
        if self.triggerArgument is not None:
            triggerArgument = builder.CreateString(self.triggerArgument)
        if self.behaviorArgument is not None:
            behaviorArgument = builder.CreateString(self.behaviorArgument)
        BossExternalBTExcelStart(builder)
        BossExternalBTExcelAddExternalBtId(builder, self.externalBtId)
        BossExternalBTExcelAddAiPhase(builder, self.aiPhase)
        BossExternalBTExcelAddExternalBtNodeType(builder, self.externalBtNodeType)
        BossExternalBTExcelAddExternalBtTrigger(builder, self.externalBtTrigger)
        if self.triggerArgument is not None:
            BossExternalBTExcelAddTriggerArgument(builder, triggerArgument)
        BossExternalBTExcelAddBehaviorRate(builder, self.behaviorRate)
        BossExternalBTExcelAddExternalBehavior(builder, self.externalBehavior)
        if self.behaviorArgument is not None:
            BossExternalBTExcelAddBehaviorArgument(builder, behaviorArgument)
        bossExternalBtexcel = BossExternalBTExcelEnd(builder)
