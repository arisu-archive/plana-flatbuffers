// Code generated by fbsgen. DO NOT EDIT.
// Package excel contains generated code for a FlatBuffers schema.
package excel

import (
	fbsutils "github.com/arisu-archive/bluearchive-fbs-utils"
	flatbuffers "github.com/google/flatbuffers/go"
)

// EventContentPlayGuideExcelDto represents a FlatBuffers table.
type EventContentPlayGuideExcelDto struct {
	fbsutils.FlatBuffer
	ID             int64  `json:"id"`
	EventContentID int64  `json:"event_content_id"`
	IsPcBuild      bool   `json:"is_pc_build"`
	DisplayOrder   int32  `json:"display_order"`
	GuideTitle     string `json:"guide_title"`
	GuideImagePath string `json:"guide_image_path"`
	GuideText      string `json:"guide_text"`
}

// MarshalModel marshals the struct into a FlatBuffers offset.
func (t *EventContentPlayGuideExcelDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	guideTitleOffset := b.CreateString(t.GuideTitle)
	guideImagePathOffset := b.CreateString(t.GuideImagePath)
	guideTextOffset := b.CreateString(t.GuideText)
	EventContentPlayGuideExcelStart(b)
	EventContentPlayGuideExcelAddId(b, t.ID)
	EventContentPlayGuideExcelAddEventContentId(b, t.EventContentID)
	EventContentPlayGuideExcelAddIsPcBuild(b, t.IsPcBuild)
	EventContentPlayGuideExcelAddDisplayOrder(b, t.DisplayOrder)
	EventContentPlayGuideExcelAddGuideTitle(b, guideTitleOffset)
	EventContentPlayGuideExcelAddGuideImagePath(b, guideImagePathOffset)
	EventContentPlayGuideExcelAddGuideText(b, guideTextOffset)
	return EventContentPlayGuideExcelEnd(b)
}

// Marshal marshals the struct into a FlatBuffers buffer.
func (t *EventContentPlayGuideExcelDto) Marshal() ([]byte, error) {
	b := flatbuffers.NewBuilder(0)
	b.Finish(t.MarshalModel(b))
	return b.FinishedBytes(), nil
}

// UnmarshalMessage unmarshals the struct from a FlatBuffers table.
func (t *EventContentPlayGuideExcelDto) UnmarshalMessage(e *EventContentPlayGuideExcel) error {
	t.ID = e.Id()
	t.EventContentID = e.EventContentId()
	t.IsPcBuild = e.IsPcBuild()
	t.DisplayOrder = e.DisplayOrder()
	t.GuideTitle = string(e.GuideTitle())
	t.GuideImagePath = string(e.GuideImagePath())
	t.GuideText = string(e.GuideText())
	return nil
}

// Unmarshal unmarshals the struct from a FlatBuffers buffer.
func (t *EventContentPlayGuideExcelDto) Unmarshal(data []byte) error {
	root := GetRootAsEventContentPlayGuideExcel(data, 0)
	return t.UnmarshalMessage(root)
}

// FlatDataName returns the FlatBuffers table name.
func (EventContentPlayGuideExcelDto) FlatDataName() string {
	return "EventContentPlayGuideExcel"
}
