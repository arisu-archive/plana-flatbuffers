// Code generated by fbsgen. DO NOT EDIT.
// Package excel contains generated code for a FlatBuffers schema.
package excel

import (
	fbsutils "github.com/arisu-archive/bluearchive-fbs-utils"
	flatbuffers "github.com/google/flatbuffers/go"
)

// EventContentCardExcelDto represents a FlatBuffers table.
type EventContentCardExcelDto struct {
	fbsutils.FlatBuffer
	CardGroupID      int32        `json:"card_group_id"`
	EventContentID   int64        `json:"event_content_id"`
	LocalizeEtcID    uint32       `json:"localize_etc_id"`
	IconPath         string       `json:"icon_path"`
	BackIconPath     string       `json:"back_icon_path"`
	RewardParcelType []ParcelType `json:"reward_parcel_type"`
	RewardParcelID   []int64      `json:"reward_parcel_id"`
}

// MarshalModel marshals the struct into a FlatBuffers offset.
func (t *EventContentCardExcelDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	iconPathOffset := b.CreateString(t.IconPath)
	backIconPathOffset := b.CreateString(t.BackIconPath)
	EventContentCardExcelStartRewardParcelTypeVector(b, len(t.RewardParcelType))
	for i := len(t.RewardParcelType) - 1; i >= 0; i-- {
		b.PrependInt32(int32(t.RewardParcelType[i]))
	}
	rewardParcelTypeOffset := b.EndVector(len(t.RewardParcelType))
	EventContentCardExcelStartRewardParcelIdVector(b, len(t.RewardParcelID))
	for i := len(t.RewardParcelID) - 1; i >= 0; i-- {
		b.PrependInt64(t.RewardParcelID[i])
	}
	rewardParcelIDOffset := b.EndVector(len(t.RewardParcelID))
	EventContentCardExcelStart(b)
	EventContentCardExcelAddCardGroupId(b, t.CardGroupID)
	EventContentCardExcelAddEventContentId(b, t.EventContentID)
	EventContentCardExcelAddLocalizeEtcId(b, t.LocalizeEtcID)
	EventContentCardExcelAddIconPath(b, iconPathOffset)
	EventContentCardExcelAddBackIconPath(b, backIconPathOffset)
	EventContentCardExcelAddRewardParcelType(b, rewardParcelTypeOffset)
	EventContentCardExcelAddRewardParcelId(b, rewardParcelIDOffset)
	return EventContentCardExcelEnd(b)
}

// Marshal marshals the struct into a FlatBuffers buffer.
func (t *EventContentCardExcelDto) Marshal() ([]byte, error) {
	b := flatbuffers.NewBuilder(0)
	b.Finish(t.MarshalModel(b))
	return b.FinishedBytes(), nil
}

// UnmarshalMessage unmarshals the struct from a FlatBuffers table.
func (t *EventContentCardExcelDto) UnmarshalMessage(e *EventContentCardExcel) error {
	t.CardGroupID = e.CardGroupId()
	t.EventContentID = e.EventContentId()
	t.LocalizeEtcID = e.LocalizeEtcId()
	t.IconPath = string(e.IconPath())
	t.BackIconPath = string(e.BackIconPath())
	t.RewardParcelType = make([]ParcelType, e.RewardParcelTypeLength())
	for i := 0; i < e.RewardParcelTypeLength(); i++ {
		t.RewardParcelType[i] = e.RewardParcelType(i)
	}
	t.RewardParcelID = make([]int64, e.RewardParcelIdLength())
	for i := 0; i < e.RewardParcelIdLength(); i++ {
		t.RewardParcelID[i] = e.RewardParcelId(i)
	}
	return nil
}

// Unmarshal unmarshals the struct from a FlatBuffers buffer.
func (t *EventContentCardExcelDto) Unmarshal(data []byte) error {
	root := GetRootAsEventContentCardExcel(data, 0)
	return t.UnmarshalMessage(root)
}

// FlatDataName returns the FlatBuffers table name.
func (EventContentCardExcelDto) FlatDataName() string {
	return "EventContentCardExcel"
}
