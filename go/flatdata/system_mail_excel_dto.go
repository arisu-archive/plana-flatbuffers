// Code generated by fbsgen. DO NOT EDIT.
// Package flatdata contains generated code for a FlatBuffers schema.
package flatdata

import (
	fbsutils "github.com/arisu-archive/bluearchive-fbs-utils"
	flatbuffers "github.com/google/flatbuffers/go"
)

// SystemMailExcelDto represents a FlatBuffers table.
type SystemMailExcelDto struct {
	fbsutils.FlatBuffer
	MailType             MailType `json:"mail_type"`
	IsProductMail        bool     `json:"is_product_mail"`
	IsVariableExpiredDay bool     `json:"is_variable_expired_day"`
	ExpiredDay           int64    `json:"expired_day"`
	Sender               string   `json:"sender"`
	Comment              string   `json:"comment"`
}

// MarshalModel marshals the struct into a FlatBuffers offset.
func (t *SystemMailExcelDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("SystemMail"))
	}
	senderOffset := b.CreateString(fbsutils.Encode(t.Sender, t.FlatBuffer.TableKey))
	commentOffset := b.CreateString(fbsutils.Encode(t.Comment, t.FlatBuffer.TableKey))
	SystemMailExcelStart(b)
	SystemMailExcelAddMailType(b, MailType(fbsutils.Encode(int32(t.MailType), t.FlatBuffer.TableKey)))
	SystemMailExcelAddIsProductMail(b, t.IsProductMail)
	SystemMailExcelAddIsVariableExpiredDay(b, t.IsVariableExpiredDay)
	SystemMailExcelAddExpiredDay(b, fbsutils.Encode(t.ExpiredDay, t.FlatBuffer.TableKey))
	SystemMailExcelAddSender(b, senderOffset)
	SystemMailExcelAddComment(b, commentOffset)
	return SystemMailExcelEnd(b)
}

// Marshal marshals the struct into a FlatBuffers buffer.
func (t *SystemMailExcelDto) Marshal() ([]byte, error) {
	b := flatbuffers.NewBuilder(0)
	b.Finish(t.MarshalModel(b))
	return b.FinishedBytes(), nil
}

// UnmarshalMessage unmarshals the struct from a FlatBuffers table.
func (t *SystemMailExcelDto) UnmarshalMessage(e *SystemMailExcel) error {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("SystemMail"))
	}
	t.MailType = MailType(fbsutils.Decode(int32(e.MailType()), t.FlatBuffer.TableKey))
	t.IsProductMail = e.IsProductMail()
	t.IsVariableExpiredDay = e.IsVariableExpiredDay()
	t.ExpiredDay = fbsutils.Decode(e.ExpiredDay(), t.FlatBuffer.TableKey)
	t.Sender = fbsutils.Decode(string(e.Sender()), t.FlatBuffer.TableKey)
	t.Comment = fbsutils.Decode(string(e.Comment()), t.FlatBuffer.TableKey)
	return nil
}

// Unmarshal unmarshals the struct from a FlatBuffers buffer.
func (t *SystemMailExcelDto) Unmarshal(data []byte) error {
	root := GetRootAsSystemMailExcel(data, 0)
	return t.UnmarshalMessage(root)
}

// FlatDataName returns the FlatBuffers table name.
func (SystemMailExcelDto) FlatDataName() string {
	return "SystemMailExcel"
}
