// Code generated by fbsgen. DO NOT EDIT.
// Package flatdata contains generated code for FlatBuffers schema
package flatdata

import (
	"errors"
	fbsutils "github.com/arisu-archive/bluearchive-fbs-utils"
	flatbuffers "github.com/google/flatbuffers/go"
)

// InformationStrategyObjectExcelTableDto represents a FlatBuffers table
type InformationStrategyObjectExcelTableDto struct {
	fbsutils.FlatBuffer
	DataList []InformationStrategyObjectExcelDto `json:"data_list"`
}

// MarshalModel marshals the struct into flatbuffers offset
func (t *InformationStrategyObjectExcelTableDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("InformationStrategyObject"))
	}
	InformationStrategyObjectExcelTableStart(b)
	InformationStrategyObjectExcelTableStartDataListVector(b, len(t.DataList))
	for i := range len(t.DataList) {
		// The array should be reversed.
		b.PrependUOffsetT(t.DataList[len(t.DataList)-i-1].MarshalModel(b))
	}
	InformationStrategyObjectExcelTableAddDataList(b, b.EndVector(len(t.DataList)))
	return InformationStrategyObjectExcelTableEnd(b)
}

// Marshal marshals the struct into a FlatBuffers buffer
func (t *InformationStrategyObjectExcelTableDto) Marshal() ([]byte, error) {
	b := flatbuffers.NewBuilder(0)
	FinishInformationStrategyObjectExcelTableBuffer(b, t.MarshalModel(b))
	return b.FinishedBytes(), nil
}

// UnmarshalMessage unmarshals the struct from a FlatBuffers buffer
func (t *InformationStrategyObjectExcelTableDto) UnmarshalMessage(e *InformationStrategyObjectExcelTable) error {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("InformationStrategyObject"))
	}
	t.DataList = make([]InformationStrategyObjectExcelDto, e.DataListLength())
	for i := range e.DataListLength() {
		d := new(InformationStrategyObjectExcel)
		if !e.DataList(d, i) {
			return errors.New("failed to unmarshal data")
		}
		t.DataList[i].UnmarshalMessage(d)
	}
	return nil
}

// Unmarshal unmarshals the struct from a FlatBuffers buffer
func (t *InformationStrategyObjectExcelTableDto) Unmarshal(data []byte) error {
	root := GetRootAsInformationStrategyObjectExcelTable(data, 0)
	err := t.UnmarshalMessage(root)
	if err != nil {
		return err
	}
	return nil
}

// Name returns the name of the flatbuffer table name
func (InformationStrategyObjectExcelTableDto) FlatDataName() string {
	return "InformationStrategyObjectExcelTable"
}
