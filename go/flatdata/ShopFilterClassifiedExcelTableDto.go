// Code generated by fbsgen. DO NOT EDIT.
// Package flatdata contains generated code for FlatBuffers schema
package flatdata

import (
	"errors"
	fbsutils "github.com/arisu-archive/bluearchive-fbs-utils"
	flatbuffers "github.com/google/flatbuffers/go"
)

// ShopFilterClassifiedExcelTableDto represents a FlatBuffers table
type ShopFilterClassifiedExcelTableDto struct {
	fbsutils.FlatBuffer
	DataList []ShopFilterClassifiedExcelDto `json:"data_list"`
}

// MarshalModel marshals the struct into flatbuffers offset
func (t *ShopFilterClassifiedExcelTableDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("ShopFilterClassified"))
	}
	ShopFilterClassifiedExcelTableStart(b)
	ShopFilterClassifiedExcelTableStartDataListVector(b, len(t.DataList))
	for i := range len(t.DataList) {
		// The array should be reversed.
		b.PrependUOffsetT(t.DataList[len(t.DataList)-i-1].MarshalModel(b))
	}
	ShopFilterClassifiedExcelTableAddDataList(b, b.EndVector(len(t.DataList)))
	return ShopFilterClassifiedExcelTableEnd(b)
}

// Marshal marshals the struct into a FlatBuffers buffer
func (t *ShopFilterClassifiedExcelTableDto) Marshal() ([]byte, error) {
	b := flatbuffers.NewBuilder(0)
	FinishShopFilterClassifiedExcelTableBuffer(b, t.MarshalModel(b))
	return b.FinishedBytes(), nil
}

// UnmarshalMessage unmarshals the struct from a FlatBuffers buffer
func (t *ShopFilterClassifiedExcelTableDto) UnmarshalMessage(e *ShopFilterClassifiedExcelTable) error {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("ShopFilterClassified"))
	}
	t.DataList = make([]ShopFilterClassifiedExcelDto, e.DataListLength())
	for i := range e.DataListLength() {
		d := new(ShopFilterClassifiedExcel)
		if !e.DataList(d, i) {
			return errors.New("failed to unmarshal data")
		}
		t.DataList[i].UnmarshalMessage(d)
	}
	return nil
}

// Unmarshal unmarshals the struct from a FlatBuffers buffer
func (t *ShopFilterClassifiedExcelTableDto) Unmarshal(data []byte) error {
	root := GetRootAsShopFilterClassifiedExcelTable(data, 0)
	err := t.UnmarshalMessage(root)
	if err != nil {
		return err
	}
	return nil
}

// Name returns the name of the flatbuffer table name
func (ShopFilterClassifiedExcelTableDto) FlatDataName() string {
	return "ShopFilterClassifiedExcelTable"
}
