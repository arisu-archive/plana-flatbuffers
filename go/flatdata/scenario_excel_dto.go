...
func (t *ScenarioExcelDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	...
	for i := len(t.Idle) - 1; i >= 0; i-- {
		b.PrependInt32(fbsutils.Convert(int32(t.Idle[i]), t.FlatBuffer.TableKey))
	}
	idleOffset := b.EndVector(len(t.Idle))
	ScenarioExcelStart(b)
	ScenarioExcelAddNone(b, noneOffset)
	ScenarioExcelAddIdle(b, idleOffset)
	ScenarioExcelAddCafe(b, DialogCategory(fbsutils.Convert(int32(t.Cafe), t.FlatBuffer.TableKey)))
	ScenarioExcelAddTalk(b, DialogType(fbsutils.Convert(int32(t.Talk), t.FlatBuffer.TableKey)))
	ScenarioExcelAddOpen(b, StoryCondition(fbsutils.Convert(int32(t.Open), t.FlatBuffer.TableKey)))
	ScenarioExcelAddEnterConver(b, EmojiEvent(fbsutils.Convert(int32(t.EnterConver), t.FlatBuffer.TableKey)))
	ScenarioExcelAddCenter(b, ScenarioZoomAnchors(fbsutils.Convert(int32(t.Center), t.FlatBuffer.TableKey)))
	ScenarioExcelAddInstant(b, ScenarioZoomType(fbsutils.Convert(int32(t.Instant), t.FlatBuffer.TableKey)))
	ScenarioExcelAddPrologue(b, ScenarioContentType(fbsutils.Convert(int32(t.Prologue), t.FlatBuffer.TableKey)))
