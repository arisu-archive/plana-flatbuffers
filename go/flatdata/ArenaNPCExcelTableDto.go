// Code generated by fbsgen. DO NOT EDIT.
// Package flatdata contains generated code for FlatBuffers schema
package flatdata

import (
	"errors"
	fbsutils "github.com/arisu-archive/bluearchive-fbs-utils"
	flatbuffers "github.com/google/flatbuffers/go"
)

// ArenaNPCExcelTableDto represents a FlatBuffers table
type ArenaNPCExcelTableDto struct {
	fbsutils.FlatBuffer
	DataList []ArenaNPCExcelDto `json:"data_list"`
}

// MarshalModel marshals the struct into flatbuffers offset
func (t *ArenaNPCExcelTableDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("ArenaNPC"))
	}
	ArenaNPCExcelTableStart(b)
	ArenaNPCExcelTableStartDataListVector(b, len(t.DataList))
	for i := range len(t.DataList) {
		// The array should be reversed.
		b.PrependUOffsetT(t.DataList[len(t.DataList)-i-1].MarshalModel(b))
	}
	ArenaNPCExcelTableAddDataList(b, b.EndVector(len(t.DataList)))
	return ArenaNPCExcelTableEnd(b)
}

// Marshal marshals the struct into a FlatBuffers buffer
func (t *ArenaNPCExcelTableDto) Marshal() ([]byte, error) {
	b := flatbuffers.NewBuilder(0)
	FinishArenaNPCExcelTableBuffer(b, t.MarshalModel(b))
	return b.FinishedBytes(), nil
}

// UnmarshalMessage unmarshals the struct from a FlatBuffers buffer
func (t *ArenaNPCExcelTableDto) UnmarshalMessage(e *ArenaNPCExcelTable) error {
	if t.FlatBuffer.TableKey == nil {
		t.FlatBuffer.InitKey(fbsutils.CreateTableKey("ArenaNPC"))
	}
	t.DataList = make([]ArenaNPCExcelDto, e.DataListLength())
	for i := range e.DataListLength() {
		d := new(ArenaNPCExcel)
		if !e.DataList(d, i) {
			return errors.New("failed to unmarshal data")
		}
		t.DataList[i].UnmarshalMessage(d)
	}
	return nil
}

// Unmarshal unmarshals the struct from a FlatBuffers buffer
func (t *ArenaNPCExcelTableDto) Unmarshal(data []byte) error {
	root := GetRootAsArenaNPCExcelTable(data, 0)
	err := t.UnmarshalMessage(root)
	if err != nil {
		return err
	}
	return nil
}

// Name returns the name of the flatbuffer table name
func (ArenaNPCExcelTableDto) FlatDataName() string {
	return "ArenaNPCExcelTable"
}
